...
```
#### We cannot be sure that GDP has unit root or not.
...
```
#### We cannot be sure that Turkish Lira exchange rate has unit root or not.
...
```
#### We cannot be sure that log(GDP) has unit root or not.
...
```
#### We cannot be sure that log(Turkish Lira exchange rate) has unit root or not.
...
pander(fit4)
```
#### There are no significant seasonal dummy variables, so there are no seasonality.
...
```
#### Seasonal dummies are not significant and does not really matter the number of lags in NW SE estimation.
#### In quarters with one more percent US$ exchange rate difference increase the level of Turkish GDP difference lessen by an additional 13% on average relative to quarters with one less percent US$ exchange rate difference. If we take into consideration the binary variables for quarters on RHS then the results were the very similar.
